...
{
    public function add_edit_comment_evaluation(Request $request, $product_id)
    {
        ...
        if ($request->evaluation) {
            if (empty(Evaluation::where('user_id', Auth::User()->id)->where('product_id', $product_id)->first())) {
                ...
                $evaluation = Evaluation::where('user_id', Auth::User()->id)->where('product_id', $product_id)->first();
        ...
        if ($request->text) {
            if (empty(Comment::where('user_id', auth::User()->id)->where('product_id', $product_id)->first())) {
                ...
                $comment = Comment::where('user_id', Auth::User()->id)->where('product_id', $product_id)->first();
    ...
    public function get_user_comment_evaluation($product_id)
    {
        $comment = Comment::where('user_id', Auth::user()->id)->where('product_id', $product_id)->get(['id', 'text']);
        $evaluation = Evaluation::where('user_id', Auth::user()->id)->where('product_id', $product_id)->get(['id', 'evaluation']);
